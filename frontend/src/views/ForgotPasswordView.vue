  ...
  <div class="auth-shell">
    <div class="auth-card">
      ...
      <template v-else>
        ...
        <Message v-if="error" severity="error" :closable="false" class="full-width" style="margin-bottom: 1rem">{{ error }}</Message>
        <form @submit.prevent="submit">
          <div class="field">
            <label for="email">{{ t('auth.email') }}</label>
            <InputText id="email" v-model="form.email" type="email" :placeholder="t('auth.emailPlaceholder')" class="full-width" />
          </div>
          <Button type="submit" :label="t('auth.forgotSubmit')" icon="pi pi-send" class="full-width" :loading="loading" style="margin-top: 0.5rem" />
        </form>
      </template>

      <p class="auth-footer">
        <RouterLink to="/login"><i class="pi pi-arrow-left" style="font-size: 0.75rem" /> {{ t('auth.backToLogin') }}</RouterLink>
      </p>
    </div>
  </div>
</template>
